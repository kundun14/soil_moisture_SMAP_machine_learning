...
list.files.modis = list.files(path = "./img_mod/SAVI",
                              recursive = TRUE, 
                              full.name = TRUE,
                              pattern = "\\.tif$")
...
fechas_MOD = getZ(get(load("./img_mod/SAVI/SAVI.RData")))
...
writeRaster(MOD,
            filename = './raster_stacks/SAVI/SAVI_na_sort',
            overwrite= T)
...
write.table(MOD_dates, 
            "./raster_stacks/SAVI/SAVI_fechas.csv", 
            row.names = F,
            col.names = F)
...
for (i in 1:length(var_list)){
  colnames(var_list[[i]]) = "SAVI" #### cambiar nombre de variable
} 
...
write.csv(MOD_df, "./data_frames/SAVI_pre_outliers.csv", row.names=FALSE, quote=FALSE)  #guardar DF
...
MOD_df = read_csv("./data_frames/SAVI_pre_outliers.csv")
