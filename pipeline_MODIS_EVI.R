...
list.files.modis = list.files(path = "./img_mod/EVI",
                              recursive = TRUE, 
                              full.name = TRUE,
                              pattern = "\\.tif$")
...
fechas_MOD = getZ(get(load("./img_mod/EVI/EVI.RData")))
...
writeRaster(MOD,
            filename = './raster_stacks/EVI/EVI_na_sort',
            overwrite= T)
...
writeRaster(MOD,
            filename = "./raster_stacks/EVI/EVI_har", 
            overwrite= T)

write.table(MOD_dates, 
            "./raster_stacks/EVI/EVI_fechas.csv", 
            row.names = F,
            col.names = F)
...
for (i in 1:length(var_list)){
  colnames(var_list[[i]]) = "EVI" #### cambiar nombre de variable
} 
...
write.csv(MOD_df, "./data_frames/EVI_pre_outliers.csv", row.names=FALSE, quote=FALSE)  #guardar DF
