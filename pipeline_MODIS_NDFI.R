...
list.files.modis = list.files(path = "./img_mod/NDFI",
                              recursive = TRUE, 
                              full.name = TRUE,
                              pattern = "\\.tif$")
...
fechas_MOD = getZ(get(load("./img_mod/NDFI/NDFI.RData")))
...
values(stack_1)[values(stack_1) > 0.30] = NA # exluir pixels saturados >0.30
...
writeRaster(MOD,
            filename = './raster_stacks/NDFI/NDFI_na_sort',
            overwrite= T)
...
values(MOD_sp)[values(MOD_sp) > 0.30] = NA # excluir zonas saturadas >0.30
...
writeRaster(MOD,
            filename = "./raster_stacks/NDFI/NDFI_har", 
            overwrite= T)

write.table(MOD_dates, 
            "./raster_stacks/NDFI/NDFI_fechas.csv", 
            row.names = F,
            col.names = F)
...
for (i in 1:length(var_list)){
  colnames(var_list[[i]]) = "NDFI" #### cambiar nombre de variable
} 
...
write.csv(MOD_df, "./data_frames/NDFI_pre_outliers.csv", row.names=FALSE, quote=FALSE)  #guardar DF
...
MOD_df = read_csv("./data_frames/NDFI_pre_outliers.csv")
...
p = ggplot(mod_arr_sub, aes(Fecha, NDFI, group = NPix)) +
  geom_line(color="purple") + 
  ggtitle(" NDFI MOD09GA \n Pre outliers detection \n Área de estudio\n 2017-2021") +
  facet_wrap(~ NPix)
